 ...
 <a href="{{URL::to('/login-index')}}" class="sign-in">Đăng nhập</a>
     ...
     <script>
     function updateFilterCheckbox(filterType) {
         let checkboxes = document.querySelectorAll(`input[name="${filterType}"]:checked`);
         let selectedValues = Array.from(checkboxes).map(checkbox => checkbox.value);

         // Cập nhật URL với tham số lọc (dùng phương thức pushState hoặc replaceState của history API)
         let url = new URL(window.location.href); // Lấy URL hiện tại
         url.searchParams.delete(filterType); // Xóa các tham số lọc cũ

         // Thêm các giá trị lọc mới vào URL
         if (selectedValues.length > 0) {
             url.searchParams.set(filterType, selectedValues.join(','));
         }

         // Cập nhật lại URL mà không làm reload trang
         var new_url = window.history.pushState({}, '', url);

         // Nếu bạn muốn reload trang ngay sau khi cập nhật URL
             ...
             $.ajax({
                 ...
                 success: function(response) {
                     if (response.length === 0) {
                         reviewContainer.html('<p>Không có sản phẩm nào.</p>');
                         return;
                     }
                     if (response.length > 0) {
                         wishlistContainer.empty();
                         response.forEach(function(item) {
                             //  console.log(item.product_favorite.product_name);
                             var wishlistItem =
                                 `<p>${item.product_favorite.product_name}</p>
                                     `;
